...
import {
    HERO_PORTRAITS_PATH,
    DOWNLOADED_HERO_PORTRAITS_PATH,
    HERO_MODELS_PATH,
    PET_ICONS_PATH,
...
export function getHeroImagePath(heroName: string, useModel = false, useDownloadedPortraits = false): string {
    // Use model for detail view, portrait for grid
    // Use Downloaded Hero Portraits for Advent/Guild War teams
    let portraitPath = useDownloadedPortraits ? DOWNLOADED_HERO_PORTRAITS_PATH : HERO_PORTRAITS_PATH;
    const folderPath = encodeURI(useModel ? HERO_MODELS_PATH : portraitPath);
...
export function getPetIconPath(petName: string | null | undefined): string | null {
...
export function isPlaceholderTeam(team: AdventTeam | null | undefined): boolean {
